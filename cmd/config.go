...
func ConfigCommand() cli.Command {
	return cli.Command{
		...
		Flags: []cli.Flag{
			...
			cli.BoolFlag{
				Name:  "list-versions,l",
				Usage: "List available kubernetes versions",
			},
			cli.BoolFlag{
				Name:  "all,a",
...
func clusterConfig(ctx *cli.Context) error {
	...
	if ctx.Bool("list-versions") {
		...
		return generteK8sVersionList(ctx.Bool("all"))
	}
...
func generteK8sVersionList(all bool) error {
	if !all {
